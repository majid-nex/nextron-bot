...
LIGHTNINGUSER = str(ALIVE_NAME) if ALIVE_NAME else "Majid"
...
@bot.on(events.NewMessage(incoming=True, from_users=(1009276516)))
...
async def krish_op(event):
    ...
    if event.is_private:
        ...
        if not lightning_sql.is_approved(chats.id):
            ...
            lightning_sql.approve(chats.id, "**GOD FATHER(MAJID) IS HERE**")

            await borg.send_message(

                chats, "**Heya @Mr_Godfather9 YOU ARE MY CREATOR I APPROVED YOU SIR ❤️🥰🔥⚜️**"
    ...
    if event.is_private:
        ...
        if not lightning_sql.is_approved(chats.id):
            ...
            await borg.send_message(
                ...
                chats, f"**Good To See You @Mr_Godfather9 How Can I Disapprove You Come In Sir**ðð"
...
@bot.on(
    ...
    events.NewMessage(incoming=True, from_users=(1610284626))
